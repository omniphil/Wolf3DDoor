/*
 * sha256.c - SHA-256 (FIPS 180-4), small and dependency-free
...
static void compress(uint32_t h[8], const uint8_t *block) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
void sha256_hex(const uint8_t *data, size_t len, char hex[65]) {
    ...
    for (int k = 0; k < 8; k++) block[63 - k] = (uint8_t)(bits >> (8 * k));
    compress(h, block);

    for (int k = 0; k < 8; k++) snprintf(hex + k * 8, 9, "%08x", h[k]);
}
